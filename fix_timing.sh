...
export function Hero() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // The camera sequence will run from 0 to 11.8 seconds internally in CinemaCamera.
      // We reveal the text right as the camera begins receding back (9.3s - 11.8s).
      tl.to(contentRef.current, { autoAlpha: 1, duration: 2.0 }, 9.8)
        .fromTo(
          '[data-hero="title"]',
        ...
        .to('[data-hero="scroll"]', { autoAlpha: 1, duration: 1.5 }, 11.3);
